...
`packg-003` implements dry-run-only `install skills --source <local-path> --dry-run` and `install templates --source <local-path> --dry-run`. A local source must contain `kkachi-install-manifest.json` with `version`, `kind`, `package`, `compat`, and item `source` / `target` / `sha256` / `owner_marker` fields. Dry runs validate checksums and path confinement, then report path actions (`create`, `update`, `unchanged`, `preserve`, `conflict`) without taking locks, appending events, or mutating target files. Real install/update remains reserved for `packg-004`; later `project` subcommands remain reserved placeholders until their roadmap tasks add real behavior.

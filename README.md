...
The current implementation covers `corex-001` through `corex-005`, `runwf-001` through `runwf-004`, `gates-001` through `gates-005`, `packg-001` through `packg-004`, `pilot-001` through `pilot-005`, `align-001` through `align-008`, `graph-001` through `graph-012`, `token-001`/`token-002`, MAR evidence, policy-promotion evidence, DESIGN-004/DESIGN-005 design-evidence schema/artifact/gate support, GAJAE-002 source-side GJC evidence wrapper MVP, GAJAE-003 packet-ref/artifact-ref validation, GAJAE-004/006 ralplan callback and watcher evidence support, GAJAE-007/008 native GJC ralplan/ultragoal adapters, and GAJAE-009 KAT evidence normalization. GAJAE source work does not authorize install/release/runtime activation by itself.
...
- `make VERSION=0.2.0 release` writes release artifacts to `dist/`:
  - `dist/kkachi-agent-helper_0.2.0_<goos>_<goarch>`
  - `dist/kkachi-agent-helper_0.2.0_<goos>_<goarch>.tar.gz`
  - `dist/SHA256SUMS`
- Tagged `go install ...@v0.2.0` builds derive the helper version from Go module build info; local `make build` defaults to `0.2.0` for this release and can still be overridden with `VERSION=<version>`.
...
The GAJAE `gjc` group is a thin deterministic wrapper around a callable `gjc` binary. Its start commands accept `--packet <run-local-packet>` as KAS packet input, but KAH does not pass `--packet` through as the live native GJC invocation shape. For `start-ralplan`, KAH reads the packet's `native_ralplan_input.stage`, `.stage_n`, and `.artifact` fields, validates the artifact as run-local regular evidence, and invokes GJC 0.7.3 as `ralplan --write --stage <stage> --stage_n <stage_n> --artifact <artifact> --json`. For `start-ultragoal`, KAH reads `native_ultragoal_input.mode=brief_file` and `.brief`, materializes a run-local `native_input_ref`, and invokes GJC 0.7.3 as `ultragoal create-goals --brief-file <native_input_ref.path> --json`. The wrapper sets `HOME=/Users/draccoon` for this local operator environment, creates or reuses a run-local `GJC_SESSION_ID`, and records candidate evidence under `.kkachi/runs/<run_id>/artifacts/gjc/session.json`, `.kkachi/runs/<run_id>/artifacts/gjc/receipt.json`, and `.kkachi/runs/<run_id>/artifacts/gjc/status.json`. JSON output uses schema `kah.gajae_gjc_delegation.v1` and includes `schema_version`, `run_id`, `task_id`, `command_kind`, `real_user_home`, `gjc_session_id`, `process`, `packet_ref`, `native_input_ref` when materialized, `receipt_ref`, `artifact_refs`, `plan`, `callback`, `current_required_actor`, `current_wait_reason`, `status_path`, `status_hash`, and `error`/`recovery_hint` when applicable. Callback evidence includes idempotency, source status hash, notification status, wake-evidence status, and `same_thread_wake_claim=false` unless a separately verified wake-evidence contract exists. `packet_ref` is KAS input packet evidence; `native_input_ref` is derived native GJC input evidence; `artifact_refs` are GJC candidate output evidence. GJC output, callbacks, and plan locks are candidate/mechanical evidence only; KAS, Blue, color gates, MAR, and final Kkachi acceptance remain outside KAH.

The wrapper fails closed for missing `gjc`, unsafe HOME/path input, missing or malformed session/status evidence, unsupported statuses, missing artifact refs, missing or drifted `packet_ref`, escape or cross-run refs, malformed checksums, checksum mismatches, malformed GJC JSON, missing plan hashes for `ralplan_ready`, callback idempotency conflicts, callback source-hash mismatches, unsupported notification/wake evidence states, and locked plan hash drift. Recovery for packet drift is to regenerate or repair the run-local KAS packet before consuming GJC status. `callback-kanban` records `no_wake_claim` when notification metadata is absent, `metadata_recorded_no_wake_claim` when metadata is present, and `missing_watcher_evidence` until explicit watcher/origin evidence exists; `lock-plan` records only a KAS-supplied accepted hash with an approval ref. GAJAE-006 callback/watcher closeout fields are productized as factual evidence, while same-thread Discord wake claims, install/release/runtime activation, and Kkachi acceptance authority remain outside KAH.

...
The GAJAE `gjc` group is a thin deterministic wrapper around a callable `gjc` binary. It sets `HOME=/Users/draccoon` for this local operator environment, creates or reuses a run-local `GJC_SESSION_ID`, invokes the selected GJC planning/execution command with the packet path, and records candidate evidence under `.kkachi/runs/<run_id>/artifacts/gjc/session.json`, `.kkachi/runs/<run_id>/artifacts/gjc/receipt.json`, and `.kkachi/runs/<run_id>/artifacts/gjc/status.json`. JSON output uses schema `kah.gajae_gjc_delegation.v1` and includes `schema_version`, `run_id`, `task_id`, `command_kind`, `real_user_home`, `gjc_session_id`, `process`, `packet_ref`, `receipt_ref`, `artifact_refs`, `plan`, `callback`, `current_required_actor`, `current_wait_reason`, `status_path`, `status_hash`, and `error`/`recovery_hint` when applicable. `packet_ref` is KAS input packet evidence; `artifact_refs` are GJC candidate output evidence. GJC output, callbacks, and plan locks are candidate/mechanical evidence only; KAS, Blue, color gates, MAR, and final Kkachi acceptance remain outside KAH.

The wrapper fails closed for missing `gjc`, unsafe HOME/path input, missing or malformed session/status evidence, unsupported statuses, missing artifact refs, missing or drifted `packet_ref`, escape or cross-run refs, malformed checksums, checksum mismatches, malformed GJC JSON, missing plan hashes for `ralplan_ready`, callback idempotency conflicts, and locked plan hash drift. Recovery for packet drift is to regenerate or repair the run-local KAS packet before consuming GJC status. `callback-kanban` records no-wake-claim when notification metadata is absent, and `lock-plan` records only a KAS-supplied accepted hash with an approval ref. `attach-kat-evidence`, watcher productization, same-thread Discord wake claims, and GAJAE-005/006 behavior remain deferred.

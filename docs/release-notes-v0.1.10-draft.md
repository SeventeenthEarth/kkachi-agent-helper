...
- `gate final` now includes workflow-instance completeness evidence: non-workflow-managed runs without `.kkachi/runs/<run_id>/workflow-instance.json` are not applicable, while workflow-managed runs fail closed when the instance is absent, selected workflow id/source metadata is missing or mismatches the instance, or existing workflow instances lack succeeded nodes and declared outputs/evidence.

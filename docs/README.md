...
Status: docs index; planning confirmation complete and workflow graph implementation evidence pending
Authority level: reading guide for KAH docs; planning records remain implementation-pending until capability/help evidence exists
...
| `docs/sot/workflow-graph.md` | Planning-confirmed SOT/spec for `.kkachi-workflow.yaml` and planned `kah graph` support | KHS proposes policy/templates; KAH validates/writes/applies; responsible approver confirmation is recorded before normative use | Confirmed planning SOT; not implemented until KAH capability/help evidence exists |
...
- Planning confirmation is recorded; implementation authority still requires capability/help evidence and any required review gates.

## Next record action

Use `docs/roadmap.md` `graph-002` as the next implementation slice for read-only graph validation and explanation, with capability/help evidence required before release behavior claims.

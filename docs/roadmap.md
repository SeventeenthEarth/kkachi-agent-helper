...
| align-003 | Command-surface capabilities report | Completed | Add `capabilities --json` with helper version, schema version, supported command groups, deprecated/omitted surfaces, and compatibility flags needed by KHS activation checks. | Implemented project-independent capabilities JSON with helper/schema versions, command inventory, KHS compatibility flags, planned phase-plan/approval flags, and omitted `install` reporting. |
| align-004 | Standard help UX | Completed | Support stable `help` / `--help` output for top-level and command groups, including required arguments, options, and documented JSON behavior. | Implemented project-independent human and structured JSON help for top-level, implemented command groups, key subcommands, `help help`, and planned `phase-plan`; unit/integration/E2E regressions cover zero-exit help outside initialized state, release artifact help, and unchanged usage errors. |

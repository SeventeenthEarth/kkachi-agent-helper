...
| Task ID | Title | Status | Work guide | Notes |
|---|---|---|---|---|
| STRICT-002 | Workflow-managed run marker and strict final-gate mode | In Progress | Add optional `workflow_managed`, `strict_workflow_order`, `selected_workflow_id`, and `workflow_source` run metadata support; final gate fails when workflow-managed runs lack a workflow instance, lack selected workflow/source metadata, carry selected workflow/source mismatches, or have incomplete/missing required node evidence. Non-workflow-managed runs retain not-applicable behavior. | Source-side implementation is present under KAH run `run-20260619T075949Z-349b40aa7c73` and pending review/final closeout; no KAS classification, workflow selection, fallback, runtime activation, install, release, push, or commit behavior is added. Focused final-gate/schema tests and docs compatibility/spec updates cover the changed contract. |

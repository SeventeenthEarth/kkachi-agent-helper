...
| marev-002 | Implement deterministic MAR artifact/gate/schema support | Source-side implemented; review pending | Adds helper validation for canonical `multi-agent-review/status.json`, MAR status vocabulary, review-role coverage state, primary/secondary/alternate/retry/premium provider attempt evidence, deterministic failure reason codes, repository-confined evidence refs, Blue disposition refs, Red adjudication refs when triggered, premium approval refs when used, `multi-agent-review-evidence` schema validation/export, and final-gate integration when MAR is required. | Source evidence: `internal/project/multi_agent_review_gate.go`, tests in `internal/project/multi_agent_review_gate_test.go`, schema/export wiring, docs updates, and `go test ./...`. KAH still must not judge finding correctness, choose review roles, run reviewers, choose retries, select alternate providers, approve waivers, or convert failed role coverage into warning-only pass. Review/release/install gates remain separate. |

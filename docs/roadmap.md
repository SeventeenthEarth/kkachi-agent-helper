...
| DAGSM-005 | Harden effective workflow capability and KAS-generated DAG compatibility | Planned | Ensure the effective KAH binary advertises workflow support consistently through version/help/capabilities; add or verify compatibility fixtures for KAS-generated run-local DAGs; document installed-runtime caveats and fail-closed behavior when workflow support is absent. | Requires DAGSM-004/WFLOW-005 acceptance. May include parser/test hardening and release/install alignment, but operational install/release activation remains separately approved. |

...
| marev-002 | Implement deterministic MAR artifact/gate/schema support | Planned | If selected, add helper validation for declared MAR artifacts: status vocabulary, reviewer coverage state, provider-attempt evidence for each required default reviewer, deterministic failure reason codes, retry linkage, approved alternate-provider evidence, explicit 주군 waiver evidence, degraded/insufficient coverage evidence, Blue disposition, Red adjudication when triggered, premium approval reference, safe paths, freshness, and read-only mutation guard evidence. | Code/tests/docs, focused gate fixtures, diagnostics/readback as needed, full verification, review gates, and final gate evidence. KAH must not judge finding correctness, run reviewers, choose retries, select alternate providers, approve waivers, or convert failed coverage into warning-only pass. |

...
Status: `GAJAE-004` source-side GJC ralplan/callback pilot evidence and `GAJAE-005` source-side ultragoal/KAT attachment evidence are implemented on top of the GAJAE-002 wrapper MVP. `GAJAE-006` productizes callback/watcher closeout fields and compatibility readback without promoting KAH into policy authority. `GAJAE-007` source-side behavior projects KAS `native_ralplan_input` packets into the installed GJC 0.7.3 native `ralplan --write` flags. `GAJAE-008` source-side behavior materializes KAS `native_ultragoal_input.mode=brief_file` / `.brief` into a run-local `native_input_ref` and invokes native GJC 0.7.3 `ultragoal create-goals --brief-file`. `GAJAE-009` lets KAH normalize KAT v0.1.0 status/summary/raw-log artifacts into bindable factual KAT evidence without requiring KAT source changes. KAH owns deterministic start/status/callback/plan-lock/KAT evidence only; KAS, Blue, color gates, MAR, and final Kkachi acceptance remain the authorities for policy, plan acceptance, review disposition, and completion.
...
`gjc` start commands require an initialized project, an existing run, a selected run-local packet ref under `.kkachi/runs/<run_id>/`, and a callable `gjc` binary. `start-ralplan` requires KAS packet fields `native_ralplan_input.stage`, `native_ralplan_input.stage_n`, and `native_ralplan_input.artifact`; KAH validates the artifact as run-local regular evidence and invokes GJC as `ralplan --write --stage <stage> --stage_n <stage_n> --artifact <artifact> --json`, never as `ralplan --write --packet <packet> --json`. `start-ultragoal` requires KAS packet fields `native_ultragoal_input.mode=brief_file` and `native_ultragoal_input.brief`; KAH materializes the brief into run-local `native_input_ref` evidence and invokes GJC as `ultragoal create-goals --brief-file <native_input_ref.path> --json`, never as `ultragoal create-goals --packet <packet> --json`. The wrapper sets `HOME=/Users/draccoon` by default for this operator environment and exports `GJC_SESSION_ID` from `.kkachi/runs/<run_id>/artifacts/gjc/session.json`; if no session evidence exists, KAH creates the deterministic id `gjc-<run_id>`. KAH records `packet_ref` as KAS input packet evidence with path and SHA-256, validates it mechanically as run-local readable regular-file evidence, and never parses packet policy. KAH never mutates shell profiles, provider settings, auth/token state, gateway state, model settings, KAB sessions, or KHS policy.

The GJC stdout receipt must be a compact JSON object with a supported candidate `status`, at least one run-local `artifact_refs` entry with `path` and `sha256:<64hex>`, and optionally `current_required_actor` / `current_wait_reason`. `artifact_refs` are GJC candidate output evidence, distinct from the KAS input `packet_ref`. For MVP receipt compatibility, KAH also accepts `artifacts` as an alias when `artifact_refs` is absent, but persisted KAH status always writes `artifact_refs`. Supported command-specific ready statuses are `deep_interview_ready`, `ralplan_ready`, and `ultragoal_ready`; common statuses are `running`, `blocked`, `failed`, and `cancelled`. Required actors are limited to `gjc`, `kas`, `color`, `mar`, `user`, `kat`, and `none`.

KAH writes `.kkachi/runs/<run_id>/artifacts/gjc/status.json` with schema `kah.gajae_gjc_delegation.v1`, `run_id`, `task_id`, `command_kind`, `real_user_home`, `gjc_session_id`, `process`, `packet_ref`, `native_input_ref` when native packet projection materializes a run-local GJC input file, `receipt_ref`, `artifact_refs`, `plan`, `callback`, `current_required_actor`, `current_wait_reason`, `status_path`, `status_hash`, `updated_at`, and `error` / `recovery_hint` on failures. Callback evidence includes `idempotency_key`, `source_status_hash`, `last_callback_status`, `notification_ref`, `notification_status`, `wake_evidence_status`, `last_notified_hash`, and `same_thread_wake_claim=false` unless an explicit future wake-evidence contract verifies origin/thread/watcher proof. `status_hash` is the checksum of the status payload before the hash field is populated, and is used as compact ledger evidence rather than final acceptance proof.

GAJAE-004/006 source-side behavior records the compact GJC receipt as `artifacts/gjc/receipt.json`, requires `ralplan_ready` to include a hashed plan artifact under `artifacts/plan/`, records callback idempotency/status/source-hash evidence through `callback-kanban`, and records KAS-supplied plan-lock hashes through `lock-plan`. `callback-kanban` stores notification metadata only, reports `notification_status` as `metadata_recorded_no_wake_claim` or `no_wake_claim`, reports `wake_evidence_status=missing_watcher_evidence`, and keeps `same_thread_wake_claim=false` unless a later separately approved wake-evidence contract validates explicit same-thread proof. `lock-plan` requires an approval reference and only binds the supplied accepted hash mechanically; KAH does not decide that a plan was accepted.

KAH fails closed for missing `gjc`, non-zero GJC exit, missing or malformed JSON receipts, missing/malformed `native_ralplan_input`, missing/malformed `native_ultragoal_input`, unsafe native ralplan artifacts, unsafe native ultragoal output paths, unsafe HOME, unsafe/escape/cross-run/missing refs, symlinked/non-regular KAT refs, missing/drifted `packet_ref`, missing/drifted `native_input_ref`, malformed or mismatched checksums, stale or mismatched `status_hash`, unsupported statuses, unsupported actors, missing/malformed session evidence, missing/malformed status evidence, status/session mismatches, missing plan artifact hashes for `ralplan_ready`, callback idempotency conflicts, callback source-hash mismatches, unsupported notification/wake evidence states, and locked plan hash drift. Recovery for missing or drifted `packet_ref` is to regenerate or repair the run-local KAS packet and rerun GJC before consuming status. `attach-kat-evidence` normalizes KAT v0.1.0 status/summary/raw-log refs into factual KAH KAT evidence and treats KAT `status_hash` as KAT self-integrity metadata, not as the GJC source status hash. GAJAE-006 closeout productizes callback/watcher status fields but same-thread Discord wake claims remain unsupported until explicit metadata/watcher evidence and tests land.
...
DESIGN-004 adds canonical `.kkachi/runs/<run_id>/design-evidence.json` bootstrap plus embedded/exported `design-evidence` schema version `design004.v1`. KAH validates deterministic shape only: required top-level sections, KAS-derived `teal_applicability` booleans, `teal_required = project_has_teal_lane && ui_ux_change`, explicit `teal_skip_reason` when Teal is not required, KAS waiver metadata field shapes (`teal_waiver_approved`, `teal_waiver_approval_ref`, `teal_waiver_scope`, `teal_waiver_expires_at`), evidence ref path shape, optional `sha256:<64hex>` checksums, and required `not_applicable` reasons. DESIGN-005 adds the built-in `design-evidence` gate, diagnostics `design_evidence` summary, selected-artifact readback for `design-evidence.json`, and final-gate integration when the run manifest declares `design-evidence.json`. DESIGN-006 adds source-side cross-repo readback examples for `kkachi_non_ui_skip`, `kkachi_teal_lane_non_ui_skip`, `sudal_ui_required`, and `doksuri_ui_required`, with KAS declarations mapped into KAH schema/gate expectations when the KAS fixture is available. KAH fails closed for missing or schema-invalid design evidence, missing Teal-required plan/spec/fidelity/screenshot/verification evidence, invalid skip/waiver evidence, unsafe refs, and warning-only downgrade attempts. KAH does not classify UI, select Teal owners, judge design quality, score screenshots, approve waivers, waive gates, activate KAB, or mutate runtime/provider/auth/profile/model settings.
...
The bundle includes redacted project config, status, event entries, project-local schema versions, run-local gate reports, graph compatibility evidence, and a selected support artifact set for the requested run. `graph_compatibility.feedback_intake` reports `status: "pass"` with effective bounds when canonical feedback intake is valid, `status: "missing"` when graph state or the policy declaration is absent, and `status: "fail"` with validation issues for stale or invalid declarations. `design_evidence` reports deterministic status, reason codes, checks, and next action for `design-evidence.json` without judging design quality or selecting Teal owners. If `--run` is omitted, the active run is used when one is recorded in `status.json`; otherwise the bundle contains project-level diagnostics only. Selected artifacts are intentionally narrower than the full artifact tree: `run-metadata.json`, `phase-plan.yaml`, intake classification, backend evidence files, test/verification/docs-update evidence, `final-report.md`, and `design-evidence.json`. Missing selected artifacts are reported in the bundle with `status: "absent"` rather than causing diagnostics export to fail. Run-scoped approval records are included as `approval_records`.

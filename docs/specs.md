...
KAH fails closed for missing `gjc`, non-zero GJC exit, missing or malformed JSON receipts, missing/malformed `native_ralplan_input`, unsafe native ralplan artifacts, unsafe HOME, unsafe/escape/cross-run/missing refs, missing/drifted `packet_ref`, malformed or mismatched checksums, stale or mismatched `status_hash`, unsupported statuses, unsupported actors, missing/malformed session evidence, missing/malformed status evidence, status/session mismatches, missing plan artifact hashes for `ralplan_ready`, callback idempotency conflicts, callback source-hash mismatches, unsupported notification/wake evidence states, and locked plan hash drift. Recovery for missing or drifted `packet_ref` is to regenerate or repair the run-local KAS packet and rerun GJC before consuming status. `attach-kat-evidence` and GAJAE-005 KAT ultragoal evidence are implemented source-side; GAJAE-006 closeout productizes callback/watcher status fields but same-thread Discord wake claims remain unsupported until explicit metadata/watcher evidence and tests land.

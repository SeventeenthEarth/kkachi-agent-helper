...
- Configurable `EXTERNAL_FEEDBACK_INTAKE` support is planned, not advertised. KHS must fail closed until a future KAH binary advertises the final support surface through capabilities and diagnostics after `graph-011`; docs, graph help, direct YAML parsing, or partial `graph-009`/`graph-010` behavior are not sufficient activation evidence. The planned contract is recorded in `docs/sot/external-feedback-intake.md`, but `graph-008` does not change command behavior, schemas, diagnostics, capabilities, or current `1..3` phase-plan validation.

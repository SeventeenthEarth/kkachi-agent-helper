...
Status: source of truth for implemented compatibility; graph init, validation/explanation, semantic diff, proposal records, approval-gated apply, non-authoritative export, and graph compatibility diagnostics are implemented
...
- Configurable `EXTERNAL_FEEDBACK_INTAKE` support is partially implemented for graph read-only validation/projection only, not advertised for activation. KAH recognizes a valid `.kkachi-workflow.yaml` `feedback_intake` declaration with `policy: "EXTERNAL_FEEDBACK_INTAKE"`, `schema_version: "external-feedback-intake/v1"`, `min_rounds: 1`, `max_rounds: 5`, `required_rounds: [1]`, and `optional_rounds: [2,3,4,5]`; stale `max3`/`1..3`, missing, duplicate, unsupported, conflicting, or round 6+ declarations fail closed. KHS must still fail closed until a future KAH binary advertises the final support surface through capabilities and diagnostics after `graph-011`; docs, graph help, direct YAML parsing, or partial `graph-009`/`graph-010` behavior are not sufficient activation evidence. Current phase-plan validation remains fixed to the implemented `1..3` behavior until `graph-010`.
...
Status: graph init, validation/explanation, semantic diff, proposal records, approval-gated apply, non-authoritative graph export, and compatibility diagnostics implemented.

...
Status: source of truth for implemented compatibility; graph init, validation/explanation, semantic diff, proposal records, approval-gated apply, non-authoritative export, graph compatibility diagnostics, and phase-plan feedback-bound validation are implemented
...
- Configurable `EXTERNAL_FEEDBACK_INTAKE` support is partially implemented for graph validation/projection and phase-plan feedback-bound validation, but not advertised for activation. KAH recognizes a valid `.kkachi-workflow.yaml` `feedback_intake` declaration with `policy: "EXTERNAL_FEEDBACK_INTAKE"`, `schema_version: "external-feedback-intake/v1"`, `min_rounds: 1`, `max_rounds: 5`, `required_rounds: [1]`, and `optional_rounds: [2,3,4,5]`; stale `max3`/`1..3`, missing, duplicate, unsupported, conflicting, or round 6+ declarations fail closed. `phase-plan validate` requires a valid graph feedback policy for declared feedback phase rows, requires paired round 1, allows optional declared rounds 2 through 5, and rejects missing graph policy instead of falling back to legacy `1..3` bounds. KHS must still fail closed until a future KAH binary advertises the final support surface through capabilities and diagnostics after `graph-011`; docs, graph help, direct YAML parsing, or partial `graph-009`/`graph-010` behavior are not sufficient activation evidence.
...
Status: graph init, validation/explanation, semantic diff, proposal records, approval-gated apply, non-authoritative graph export, compatibility diagnostics, and phase-plan feedback-bound validation implemented.
...
| `diagnostics export` `graph_compatibility` | Implemented graph support/state diagnostic evidence | Fail closed when graph support is required but `state_status` is `missing` or `fail` | Report support status, graph validation, forbidden fallback sources, and `no_direct_yaml_fallback: true` without failing diagnostics export |

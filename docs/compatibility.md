...
- Token-economy evidence support is implemented for token-001 only through the `token-economy` gate, canonical `token-economy-evidence.json` artifact, embedded/exported `token-economy-evidence` schema, and `token_economy_evidence_gate=true` capability flag. KHS/KAS must supply the evidence artifact; KAH only validates deterministic shape, repository-confined refs, checksums, markers, `not_applicable` reasons, token-001 identity, and broad-mutation approval refs. KAH does not judge English prose quality, summarize logs, manage project KAS lifecycle, activate KAB, or mutate Hermes runtime/profile/provider/auth/token/gateway/model settings.

...
Status: source of truth for implemented compatibility; graph init, validation/explanation, semantic diff, and proposal records are implemented while apply/export remain planned
...
- Workflow graph compatibility: `.kkachi-workflow.yaml` plus `kkachi-agent-helper graph init`, `graph validate`, `graph explain`, `graph diff`, and `graph propose` are supported when the effective KAH binary advertises those graph subcommands through capabilities/help. KHS chooses templates, policy, and declarative proposals; KAH initializes from `khs-default` or explicit template paths, validates/explains/diffs graph state, and records proposal evidence today, while apply/export behavior remains planned. KHS must not silently edit `.kkachi-workflow.yaml` as fallback when graph apply support is missing.
...
Status: graph init, validation/explanation, semantic diff, and proposal records implemented; graph apply/export planned.

| Surface | Compatibility state | Required KHS behavior | Required KAH behavior |
|---|---|---|---|
| `.kkachi-workflow.yaml` | Project graph file with init/validation/explanation/diff support | Treat as graph authority only after capability and SOT confirmation | Initialize/validate/explain/diff today; replacement apply only through later implemented commands and audit evidence |
| `kkachi-agent-helper graph init/validate/explain/diff/propose` | Implemented graph evidence surface | Capability-check before use; record init/validation/explanation/diff/proposal evidence | Advertise subcommands through capabilities/help before KHS may rely on them |
...
Owner: KAH/KHS compatibility record
Confirming role: Responsible approver / governance evidence record
Status: graph init/validation/explanation/diff/proposal compatibility implemented; apply/export rows are not release claims until graph mutation support evidence exists
...
Decision summary: KHS chooses templates/policy/proposals; KAH initializes from `khs-default` or explicit template paths, validates/explains/diffs graph state, and records proposal evidence today; future KAH tasks apply approved graph changes deterministically; no silent direct YAML fallback; KAB alignment is future/non-authoritative.
...
Open questions: tested release versions for graph support plus apply/export capability flags remain implementation/release tasks.
Next record action: update the matrix again when graph apply/export support is implemented and advertised.
